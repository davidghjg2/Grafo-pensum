...
namespace Grafo_pensum
{
    internal class Pensum
    {
        ...
        {
            Materia nodo = new Materia(nombre);
            Array.Resize(ref nodos, nodos.Length + 1);
            nodos[nodos.Length - 1] = nodo;
        }

        public void EnlazarNodo(string nombre, Carrera carrera, string requisito)
        {
        ...
        {
            foreach (Materia nodo in nodos)
                nodo.Visitado = false;
        }

        private Materia buscarNodo(string nombre)
        {
            foreach (Materia nodo in nodos)
            {
        ...
        private int DFS(Materia nodo, string[] orden, int indice)
        {
            if (nodo.EnProceso)
            ...
            {
                nodo.EnProceso = true;
                foreach (Materia.req dependencia in nodo.desb)
                {
                    Materia newNodo = buscarNodo(dependencia.codigos);
        ...
        private void requisitosRecursivo(Materia nodo, ref string[] requisitos)
        {
            foreach (Materia.req req in nodo.reqs)
            {
                ...
                Materia requisito = buscarNodo(req.codigos);
                requisitosRecursivo(requisito, ref requisitos);
            }
        }

        private void desbloqueadasRecursivas(Materia nodo, ref string[] desbloqueadas)
        {
            foreach (Materia.req deb in nodo.desb)
            {
                ...
                Materia desbloquea = buscarNodo(deb.codigos);
        ...
        {
            Materia nodo = buscarNodo(materia);
            if (materia == null)
        ...
        {
            Materia materia = buscarNodo(nombreMateria);
            if (materia == null)
            {
